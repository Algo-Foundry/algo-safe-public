...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  // validate fields
  const validationErrors = await withValidation(req, res, validations.reimbursedSafeMigrationValidator);
  ...
  try {
    ...
    // update db
    const updated_query = await dataSource
    ...
    return Http.resBuilder(res, Http.STATUS_OK, "Success", updated_query);
  } catch (error) {
    Logger.error(String(error), "API_PENDING_TRANSACTION_UPDATED_REIMBURSED");
    await dataSource.destroy();
